    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed right-6 bottom-24 z-50 w-14 h-14 bg-blue-500 hover:bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center transition-colors"
          aria-label="チャットを開く"
        >
        ...
        <div className="fixed right-6 bottom-24 z-50 w-80 h-[480px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-gray-200">
          ...
          >
            {hasMore && (
              <button
                onClick={loadMore}
                className="text-xs text-blue-600 hover:underline self-center mb-2"
              >
                過去のメッセージを読み込む
              </button>
            )}
          ...
          <div className="shrink-0 border-t border-gray-200 bg-white px-2 py-2 flex items-center gap-1">
            <div className="relative">
              <button
                ...
                className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-blue-600 rounded-full hover:bg-gray-100"
                aria-label="ステッカー"
              >
                😊
              </button>
            ...
            <input
              ...
              className="flex-1 text-sm px-3 py-1.5 rounded-full border border-gray-200 focus:outline-none focus:border-blue-400 bg-gray-50"
            />
            <button
              onClick={handleSend}
              disabled={!input.trim()}
              className="w-8 h-8 flex items-center justify-center bg-blue-500 hover:bg-blue-600 disabled:opacity-40 text-white rounded-full transition-colors"
              aria-label="送信"
            >
