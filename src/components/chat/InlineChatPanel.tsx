...
export default function InlineChatPanel({ threadId }: Props) {
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-gray-50">
      {/* header strip */}
      <div className="shrink-0 flex items-center justify-between px-4 py-2 bg-blue-500 text-white text-sm">
      ...
      <div ref={listRef} onScroll={onScroll} className="flex-1 overflow-y-auto px-3 py-2 flex flex-col min-h-0">
        {hasMore && (
          <button onClick={loadMore} className="text-xs text-blue-600 hover:underline self-center mb-2">
            過去のメッセージを読み込む
          </button>
        )}
