...
export default function Nav() {
  ...
  useEffect(() => {
    ...
    setAnimateIn(false);
    const t = setTimeout(() => setMenuMounted(false), 280);
  ...
  function openSheet() {
    ...
    setMenuMounted(true);
    // 2 frames to ensure element is painted before starting transition
    requestAnimationFrame(() => requestAnimationFrame(() => setAnimateIn(true)));
  }

  function closeSheet() {
    setAnimateIn(false);
    closeTimerRef.current = setTimeout(() => setMenuMounted(false), 280);
  }
  ...
  return (
    <>
      ...
      >
        <div className="flex">
          <Link href="/" className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 text-[10px] font-medium transition-colors ${isActive('/') && pathname === '/' ? 'text-blue-600' : 'text-gray-500'}`}>
          ...
          <Link href="/threads" className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 text-[10px] font-medium transition-colors ${isActive('/threads') ? 'text-blue-600' : 'text-gray-500'}`}>
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <Link href="/notifications" className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 text-[10px] font-medium transition-colors ${isActive('/notifications') ? 'text-blue-600' : 'text-gray-500'}`}>
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button
            onClick={openSheet}
            className={`flex-1 flex flex-col items-center justify-center py-3 gap-1 text-[10px] font-medium transition-colors ${menuMounted ? 'text-blue-600' : 'text-gray-500'}`}
          >
            <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={menuMounted ? 2.5 : 1.5} d="M4 6h16M4 12h16M4 18h16" />
            </svg>
            メニュー
          </button>
        </div>
      ...
      {/* ── Mobile menu sheet (bottom sheet with animation) ── */}
      {menuMounted && (
        <div className="md:hidden fixed inset-0 z-[60] flex flex-col justify-end">
          ...
          <div
            ...
            className={`relative bg-white rounded-t-2xl shadow-xl transition-transform duration-300 ease-out will-change-transform ${animateIn ? 'translate-y-0' : 'translate-y-full'}`}
